...
namespace SolarMax
{
    internal class DialogFind : Dialog
    {
        ...
        private FoundItemDelegate findCallback;
        ...
        private static FindEqualityComparer fc = new FindEqualityComparer();

        private CelestialBody currentItem;
        private int selectIndex;

        public DialogFind(IRenderer Renderer, Physics Physics, QSize ScreenSize, FoundItemDelegate FoundItemDelegate, CloseCallback CloseCallback, CelestialBody CurrentItem, QPen ForePen, QPen BorderPen, QPen BackPen)
        ...
        private void updateSearch()
        {
            if (userText.Length > MAX_LENGTH)
                userText = userText.Substring(MAX_LENGTH);
            ...
            if (userText.Length > 0)
            {
                ...
                if (result != null)
                {
                    userText = result.Item2.Substring(0, userText.Length);
