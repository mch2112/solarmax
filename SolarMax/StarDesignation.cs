...
namespace SolarMax
{
    internal sealed class StarDesignation
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(ConstellationAbbreviation) && Constellation.GenitiveNames.Keys.Contains(ConstellationAbbreviation))
            {
                this.ConstellationAbbreviation = ConstellationAbbreviation;
                this.ConstellationGenitive = Constellation.GenitiveNames[ConstellationAbbreviation];
                ...
                if (!string.IsNullOrWhiteSpace(BayerDesignator))
                {
                    ...
                    {
                        this.PrimaryDesignation = string.Format("{0}{1} {2}", this.BayerDesignator, this.BayerIndex.ToSuperscript(), this.ConstellationGenitive);
                        this.ShortDesignation = string.Format("{0}{1}{2}", this.BayerDesignator, this.BayerIndex.ToSuperscript(), this.ConstellationAbbreviation);
                    }
                    else
                    {
                        this.PrimaryDesignation = string.Format("{0} {1}", this.BayerDesignator, this.ConstellationGenitive);
                        this.ShortDesignation = string.Format("{0}{1}", this.BayerDesignator, this.ConstellationAbbreviation);
                    }
                    if (this.FlamsteedNumber > 0)
                    {
                        this.SecondaryDesignation = string.Format("{0} {1}", this.FlamsteedNumber, this.ConstellationGenitive);
                    }
                    this.HasBayerOrFlamsteedDesignation = true;
                }
                else if (FlamsteedNumber > 0)
                {
                    this.PrimaryDesignation = string.Format("{0} {1}", this.FlamsteedNumber, this.ConstellationGenitive);
                    this.ShortDesignation = string.Format("{0}{1}", this.FlamsteedNumber, this.ConstellationAbbreviation);
