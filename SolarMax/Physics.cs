...
namespace SolarMax
{
    ...
    internal sealed partial class Physics
    {
        ...
        private readonly DateTime EPOCH_START = new DateTime(/*EPOCH START: DO NOT CHANGE*/ 2011, 01, 01, 0, 0, 0);
        ...
        {
            Thread t = new Thread(new ThreadStart(this.moveObjects));
            t.Priority = Priority;
            t.Name = "Physics";
            t.Start();
        }
        ...
        {
            List<Tuple<Orbiter, Orbiter, double>> list = new List<Tuple<Orbiter, Orbiter, double>>(Ephemeris.STARTING_NUM_ORBITERS * Ephemeris.STARTING_NUM_ORBITERS);
                ...
                foreach (var oo in AllOrbiters)
                    if (!o.Equals(oo))
                    {
                        list.Add(new Tuple<Orbiter, Orbiter, double>(o,                                                 // body being influenced
                                                                     oo,                                                // influencing body
                                                                     oo.MG / o.Position.DistanceToSquared(oo.Position)) // accelleration scalar
                                 );
                    }
            ...
            //var q = gravitationalInfluences.GroupBy(g => g.Item4).Select(g => new { Val = (double)g.Key, Num = (double)g.Count() }).ToList();
            //var qq = q.Select(qqq => qqq.Num / qqq.Val).ToList();
            //giIntensity = qq.Sum();
            //giRatio = giIntensity / (AllOrbiters.Length * (AllOrbiters.Length + 1));
            //var report = string.Join(Environment.NewLine, gravitationalInfluences.Select(g => g.ToString()));
        }
        //private double giRatio;
        //private double giIntensity;
        private static int[] integrationFactors = new int[256];
        private static int[] integrationFactorCount;
        private int stepCount = 0;
        
        public void UpdateAccellerationVariable()
        {
            unchecked
            {
                foreach (var o in AllOrbiters)
