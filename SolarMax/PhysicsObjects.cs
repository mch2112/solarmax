...
internal sealed partial class Physics
{
    ...
    private void CreateOrbiters()
    {
        ...
        List<Orbiter> oo = new(Ephemeris.STARTING_NUM_ORBITERS);
    ...
    private void LoadEphemeres(bool AllowLoadSnapshot)
    {
        ...
        foreach (var fi in IO.EnumerateFiles("ephemeris*.txt"))
        {
            if (fi.Name != Ephemeris.CATALOG_FILENAME)
            {
                Ephemeris e = new(IO.ReadFile(fi.FullName, DirectoryLocation.Data));
    ...
    private void SetupConstellations()
    {
        ...
        for (int i = 0; i <= Data.GetUpperBound(0); i++)
        {
            ...
            for (int j = 1; j <= Data.GetUpperBound(1); j++)
            {
                if (Data[i, j] == null)
                    break;
                c.WithLines(index, [.. Data[i, j].Trim().Split('-').Select(n => n.ParseInt(0))]);
            }
        }
