...
internal class Shape
{
    ...
    public static Shape GetCube(double SizeLength)
    {
        ...
        rs.AddLines(l, [ new( SizeLength,  SizeLength, -SizeLength),
                         new(-SizeLength,  SizeLength, -SizeLength),
                         new(-SizeLength, -SizeLength, -SizeLength),
                         new(-SizeLength, -SizeLength,  SizeLength),
                         new( SizeLength, -SizeLength,  SizeLength),
                         new( SizeLength,  SizeLength,  SizeLength),
    ...
    public static Shape GetSphere(int NumPointsPerCircle, int NumLongitudeLines, int NumLatitudeLines, double Radius, int NumRings = 0, double Oblateness = 0, bool HighlightPrimeMeridian = false, bool HighlightAntiPrimeMeridian = false, bool HighlightEquator = false, bool Hemisphere = false)
    {
        ...
        for (int i = 0; i < NumLongitudeLines; i++)
        {
            ...
            if ((HighlightPrimeMeridian && i == 0) || (HighlightAntiPrimeMeridian && i == NumLongitudeLines / 2))
    ...
    private static void AddGlyphs(Shape Target, params Shape[] Glyphs)
    {
        ...
        Target.AddLines(Glyphs[^1].Copy().Lines);
    }
    public override string ToString()
    {
        return $"Shape: {this.lines.Count} Lines {this.lines.Where(l => l is Line).Count()} Normal {this.lines.Where(l => l is LineContinuation).Count()} Continuations";
    }
}
