...
internal class FindDialog : Dialog
{
    ...
    public FindDialog(IRenderer Renderer, Physics Physics, QSize ScreenSize, FoundItemDelegate FoundItemDelegate, CloseCallback CloseCallback, QPen ForePen, QPen BorderPen, QPen BackPen)
        : base(Renderer, ScreenSize, CloseCallback, ForePen, BorderPen, BackPen)
    {
        physics = Physics;
        physics.PreloadSearch();
        findCallback = FoundItemDelegate;
    ...
    {
        rect = screenSize.GetRectangleDockedBottomRight(new QSize(340, 230), 20);
